...
def read_in_matrices(
    dataset, z_dim, algorithm, shuffled_data=False, load_weights=False
):
    """
    ...
    read_weights - a boolean if the weight matrices should be loaded instead
                   of the training and testing z matrices
    ...
    dataset_dir = os.path.join(
        "{}_{}".format(dataset, results_dir), "ensemble_z_matrices"
    )
...
def get_svcca_across_algorithm_stability(
    z_dict, algorithms, train_or_test=None, threshold=0.98
):
    """
    ...
    train_or_test - a string that indicates if the comparison should be
                    training or testing z matrices (use None for weight matrix)
            ...
            if model_a != model_b:
                ...
                for algorithm_a in algorithms:
                    for algorithm_b in algorithms:
                        ...
                        result = cca.robust_cca_similarity(
                            z_a.T, z_b.T, verbose=False, threshold=threshold
                        )
...
def get_svcca_across_z_stability(
    z_dict_a, z_dict_b, algorithm, train_or_test=None, threshold=0.98
):
    """
