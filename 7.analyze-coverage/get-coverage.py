...
def assign_attributes(df, dataset, z_dim, geneset):
    ...
    """
    df_copy = df.copy()
    return df_copy.assign(dataset=dataset, z_dim=z_dim, geneset_name=geneset)
...
base_dir = os.path.join(
    "..", "6.analyze-weights", "results", dataset, geneset, "signal"
)
...
for file in os.listdir(base_dir):
    ...
    model_coverage_df = (
        (top_df.groupby(individual_models).geneset.nunique() / n_gmt)
        .reset_index()
    ...
    ensemble_coverage_df = (
        (top_df.groupby(ensemble_models).geneset.nunique() / n_gmt)
        .reset_index()
        .rename({"geneset": "geneset_coverage"}, axis="columns")
    )
    ...
    all_coverage_df = (
        (top_df.groupby(all_models).geneset.nunique() / n_gmt)
        .reset_index()
        .rename({"geneset": "geneset_coverage"}, axis="columns")
    )
    ...
    model_zscore_summary_df = (
        top_df.groupby(individual_models).apply(
            lambda x: x.abs_top_z_score.sort_values(ascending=False).head(1)
        )
    ).reset_index()

    # Also get coverage per ensemble model (algorithm, model_type)
    ensemble_zscore_summary_df = (
        top_df.groupby(ensemble_models).apply(
            lambda x: x.abs_top_z_score.sort_values(ascending=False).head(1)
        )
    ).reset_index()

    # And, lastly, get coverage for ALL models
    all_zscore_summary_df = top_df.groupby(all_models).apply(
        lambda x: x.abs_top_z_score.sort_values(ascending=False).head(1)
    )
    all_zscore_summary_df.columns = ["abs_top_z_score"]

    # Compile all results together
    model_results_df = model_coverage_df.merge(
        model_zscore_summary_df, on=individual_models
    )

    ensemble_results_df = ensemble_coverage_df.merge(
        ensemble_zscore_summary_df, on=ensemble_models
    )

    all_results_df = all_coverage_df.merge(all_zscore_summary_df, on=all_models)

    # Track additional metrics
    model_results_df = assign_attributes(
        df=model_results_df, dataset=dataset, z_dim=z_dim, geneset=geneset
    )
    ensemble_results_df = assign_attributes(
        df=ensemble_results_df, dataset=dataset, z_dim=z_dim, geneset=geneset
    )
    all_results_df = assign_attributes(
        df=all_results_df, dataset=dataset, z_dim=z_dim, geneset=geneset
    )

    top_df = assign_attributes(df=top_df, dataset=dataset, z_dim=z_dim, geneset=geneset)
