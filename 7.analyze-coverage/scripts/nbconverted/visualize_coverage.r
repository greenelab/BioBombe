...
plot_info_list <- list(c("TCGA", "GpC5BP"), c("TCGA", "GpC4CM"), c("GTEX", "GpXCELL"))
plot_list <- list()
...
for (plot_info in plot_info_list) {
    ...
    plot_list[[plot_identifier]][['model_gg']] <-
        ggplot(model_df, aes(x = z_dim, y = geneset_coverage * 100)) +
            geom_boxplot(aes(color = algorithm), outlier.size = 0.15, lwd = 0.23) +
            coverage_theme +
    ...
    plot_list[[plot_identifier]][['ensemble_gg']] <-
        ggplot(ensemble_df, aes(x = z_dim, y = geneset_coverage * 100)) +
            geom_point(aes(color = algorithm, size = log10(abs_top_z_score)), alpha = 0.3) +
            geom_point(aes(color = algorithm), size = 0.15) +
            coverage_theme +
            scale_color_manual(name = "Algorithm",
                               values = c("#e41a1c",
                                          "#377eb8",
...
gtex_label <- cowplot::ggdraw() +
    cowplot::draw_label(label = 'GTEX\nxCell Cell Types',
                        angle = 90)
dataset_main <- cowplot::plot_grid(tcga_go_label, tcga_cm_label, gtex_label, nrow = 3)

main_plot <- (
    cowplot::plot_grid(
        plot_list[["TCGAGpC5BP"]][['model_gg']] + xlab('') + theme(legend.position = "none"),
        plot_list[['TCGAGpC5BP']][['ensemble_gg']] + xlab('') + theme(legend.position = "none"),
        plot_list[['TCGAGpC5BP']][['all_gg']] + xlab('') +
        ...
        plot_list[["TCGAGpC4CM"]][['model_gg']] + xlab('') + theme(legend.position = "none") + ggtitle(""),
        plot_list[['TCGAGpC4CM']][['ensemble_gg']] + xlab('') + theme(legend.position = "none") + ggtitle(""),
        plot_list[['TCGAGpC4CM']][['all_gg']] + xlab('') + theme(legend.position = "none") + ggtitle(""),
        plot_list[["GTEXGpXCELL"]][['model_gg']] + theme(legend.position = "none") + ggtitle(""),
        plot_list[['GTEXGpXCELL']][['ensemble_gg']] + theme(legend.position = "none") + ggtitle(""),
        plot_list[['GTEXGpXCELL']][['all_gg']] + theme(legend.position = "none") + ggtitle(""),
...
main_plot <- cowplot::plot_grid(main_plot, legend, rel_widths = c(1, 0.08), ncol = 2)
main_plot <- cowplot::plot_grid(dataset_main, main_plot, ncol = 2, rel_widths = c(0.04, 1))
