"""
2019 Gregory Way
9.tcga-classify/classify-with-top-features.py

Compare lasso with 200 features against lasso with top 200 features generated by
each algorithm.

Usage:

    python classify-with-top-features.py
...
x_matrix_dict = build_top_feature_dictionary(algorithms=algorithms, genes=genes)
...
for gene_name in genes:
    ...
    for algorithm in algorithms:
        train_df = x_matrix_dict[algorithm][gene_name]['train']
        test_df = x_matrix_dict[algorithm][gene_name]['test']
        ...
        print(
            "Training model... gene: {}, "
            "algorithm: {}".format(gene_name, algorithm)
        )
        ...
        train_metrics_, train_roc_df, train_pr_df = summarize_results(
            y_train_results, gene_name, 'real', 'top_features', 'any', algorithm, "train"
        )
        test_metrics_, test_roc_df, test_pr_df = summarize_results(
            y_test_results, gene_name, 'real', 'top_features', 'any', algorithm, "test"
        )
        cv_metrics_, cv_roc_df, cv_pr_df = summarize_results(
            y_cv_results, gene_name, 'real', 'top_features', 'any', algorithm, "cv"
        )
