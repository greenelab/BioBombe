"""
2018 Gregory Way
9.tcga-classify/classify-with-all-features.py

Predicting 33 different cancer-types using elastic net logistic regression and
compressed gene expression features in TCGA PanCanAtlas using a single ensemble model
built with all compressed features across k and across algorithms
...
coefficients for every compression model trained in their ability to predict mutation
and cancer-type.
"""
import os
import numpy as np
...
algorithm_dict = load_ensemble_dict(
    zs=zs, seeds=seeds, algorithm=algorithm, use_all_features=True
)
...
for gene_idx, gene_series in genes_df.iterrows():
    ...
    gene_dir = os.path.join("results", "mutation_ensemble_all", gene_name)
    ...
    if algorithm:
        ...
        update_name = "{}_ensemble_all_features".format(gene_name)
    ...
    for signal in algorithm_dict.keys():
        signal_dict = algorithm_dict[signal]

        z_train_file = signal_dict["train"]
        z_test_file = signal_dict["test"]
        ...
        print(
            "Training model... gene: {}, "
            "ensemble algorithm: {}, signal: {}".format(gene_name, algorithm, signal)
        )
        ...
        # Get coefficients
        if algorithm:
            algorithm_assign = "{}_feature_ensemble".format(algorithm)
        else:
            algorithm_assign = "all_feature_ensemble"
    ...
    if not algorithm:
        gene_name = "{}_ensemble_all_features".format(gene_name)
...
for acronym in sample_freeze_df.DISEASE.unique():
    ...
    cancertype_dir = os.path.join("results", "cancer-type_ensemble_all", acronym)
    ...
    if algorithm:
        ...
        update_name = "{}_ensemble_all_features".format(acronym)
        check_file = os.path.join(
    ...
    for signal in algorithm_dict.keys():
        ...
        print(
            ...
            "ensemble algorithm: {}, signal: {}".format(acronym, algorithm, signal)
        )
    ...
    if not algorithm:
        acronym = "{}_ensemble_all_features".format(acronym)
